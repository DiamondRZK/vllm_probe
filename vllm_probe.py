...
LATENCY_PROFILES = {
    ...
    "custom": {
        "delay_ms": None,    # Set via --delay-ms
        "jitter_ms": None,   # Set via --jitter-ms
        "description": "User-defined latency profile",
        "real_world": "User-specified",
    },
}
...
WORKLOAD_SCENARIOS = {
    ...
    "custom": {
        "input_tokens": None,   # Set via --input-tokens
        "output_tokens": None,  # Set via --output-tokens
        "concurrency": None,    # Set via --concurrent
        "description": "User-defined scenario with custom parameters",
        "bottleneck": "User-defined",
        "sensitivity": {
            "network": "Depends on output token count",
            "nvlink": "Depends on TP configuration",
            "vram": "Depends on input length and concurrency",
        },
        "justification": (
            "Custom scenario allowing user-specified input tokens, output tokens, "
            "and concurrency. Use when the pre-built scenarios do not match your "
            "production workload."
        ),
    },
}
...
class BenchmarkResult:
    ...
    # Per-round granularity (v1.1.0)
    warmup_round_data: list[dict] = field(default_factory=list)
    benchmark_round_data: list[dict] = field(default_factory=list)
...
def generate_prompt_token_ids(
    ...
    """
    Generate prompt token IDs using one of three modes:

    1. prompt_file: Tokenize a real .txt file. Most realistic.
    2. random_input: Truly random token IDs. No prefix caching shortcuts.
    3. default: Repeated sentence tokenized and truncated. Deterministic.

    Returns a list of integer token IDs of exactly `num_tokens` length.
    """
    import random
    ...
    if config.prompt_file:
        # ── Mode 1: Real document ────────────────────────────────────────
        prompt_path = Path(config.prompt_file)
        if not prompt_path.exists():
            raise FileNotFoundError(f"Prompt file not found: {config.prompt_file}")
        ...
        if num_tokens and len(token_ids) > num_tokens:
            ...
            logger.info(
                f"Prompt file tokenized: {len(token_ids)} tokens "
                f"(truncated from {len(tokenizer.encode(text))} to {num_tokens})"
            )
        elif num_tokens and len(token_ids) < num_tokens:
            logger.warning(
                f"Prompt file has only {len(token_ids)} tokens but "
                f"{num_tokens} requested. Using file length as-is."
            )
        else:
            logger.info(f"Prompt file tokenized: {len(token_ids)} tokens")

        return token_ids

    elif config.random_input:
        # ── Mode 2: Random token IDs ─────────────────────────────────────
        random.seed(config.seed)
        vocab_size = tokenizer.vocab_size
        # Skip IDs 0-99 and last 100 to avoid special tokens (BOS, EOS, PAD)
        low = min(100, vocab_size - 1)
        high = max(low + 1, vocab_size - 100)
        token_ids = [random.randint(low, high) for _ in range(num_tokens)]
        logger.info(
            f"Generated {len(token_ids)} RANDOM token IDs "
            f"(seed={config.seed}, vocab_size={vocab_size})"
        )
        return token_ids

    else:
        # ── Mode 3: Repeated sentence (default) ─────────────────────────
        base_sentence = (
        ...
        base_ids = tokenizer.encode(base_sentence)
        repetitions = (num_tokens // len(base_ids)) + 1
        token_ids = (base_ids * repetitions)[:num_tokens]
        logger.info(
            f"Generated {len(token_ids)} REPEATED token IDs "
            f"(base_len={len(base_ids)}, repeated {repetitions}x)"
        )
        return token_ids
...
def run_benchmark(config: ProbeConfig, logger: logging.Logger) -> BenchmarkResult:
    ...
    try:
        ...
        llm = LLM(
            ...
            enable_prefix_caching=False,  # No KV-cache reuse between rounds for honest TTFT
        )
    ...
    sampling_params = SamplingParams(
        ...
        ignore_eos=True,    # Force exact output token count — never stop early at EOS
        temperature=0.0,    # Deterministic for reproducibility
        top_p=1.0,          # No nucleus sampling — use full distribution
    )

    prompt_token_ids = generate_prompt_token_ids(
        scenario["input_tokens"], llm, config, logger,
    )
    ...
    for i in range(config.warmup_rounds):
        ...
        if scenario["concurrency"] > 1:
            wm = run_concurrent_requests(llm, sampling_params, prompt_token_ids, scenario["concurrency"], logger)
        else:
            wm = [run_single_request(llm, sampling_params, prompt_token_ids, logger)]
        # Record warmup data for transparency
        for m in wm:
            warmup_data.append({
                "round": i + 1,
                "ttft_s": round(m.get("ttft", 0), 4),
                "tpot_s": round(m.get("tpot", 0), 6),
                "total_time_s": round(m.get("total_time", 0), 4),
                "output_tokens_generated": m.get("num_tokens", 0),
                "note": "DISCARDED — " + ("CUDA JIT + clock ramp + NCCL init" if i == 0 else "hardware stabilization"),
            })
    ...
    for i in range(config.benchmark_rounds):
        ...
        if scenario["concurrency"] > 1:
            measurements = run_concurrent_requests(
                llm, sampling_params, prompt_token_ids, scenario["concurrency"], logger,
            )
        else:
            measurements = [run_single_request(llm, sampling_params, prompt_token_ids, logger)]
        ...
        for m in measurements:
            ...
            round_data.append({
                ...
                "ttft_s": round(m.get("ttft", 0), 4),
                "tpot_s": round(m.get("tpot", 0), 6),
                "throughput_tok_s": round(m.get("throughput", 0), 2),
                "total_time_s": round(m.get("total_time", 0), 4),
                "output_tokens_generated": m.get("num_tokens", 0),
            })
    ...
    # Store per-round data for JSON report transparency
    result.warmup_round_data = warmup_data
    result.benchmark_round_data = round_data
...
def generate_report(result: BenchmarkResult, config: ProbeConfig, logger: logging.Logger, simulator: PhysicsSimulator = None) -> str:
    ...
    report = {
        ...
        "configuration": {
            ...
            "random_input": config.random_input,
            "seed": config.seed if config.random_input else None,
            "prompt_file": config.prompt_file,
            "warmup_rounds": config.warmup_rounds,
            "benchmark_rounds": config.benchmark_rounds,
        },
        "results": {
            "warmup_rounds": result.warmup_round_data,
            "rounds": result.benchmark_round_data,
            "aggregate": {
                "ttft": {
            ...
            "analysis": {
                ...
                "round_variance_pct": _compute_variance(result),
            },
...
def _compute_variance(result: BenchmarkResult) -> float:
    """Compute coefficient of variation across benchmark rounds."""
    if not result.benchmark_round_data or len(result.benchmark_round_data) < 2:
        return 0.0
    times = [r["total_time_s"] for r in result.benchmark_round_data if "total_time_s" in r]
    ...
    std = variance ** 0.5
    return round(std / mean * 100, 2)  # Coefficient of variation as percentage
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  # Custom scenario with your own parameters\n"
            "  vllm_probe --scenario custom --input-tokens 4096 --output-tokens 1024 --concurrent 4 --tp 2\n\n"
            "  # Custom network delay with real document input\n"
            "  vllm_probe --scenario custom --prompt-file doc.txt --output-tokens 512 --network custom --delay-ms 75\n\n"
            "  # Random input for realistic prefill (no prefix caching)\n"
            "  vllm_probe --scenario analyst --random-input --seed 42 --tp 1\n\n"
            "  # Select specific GPUs and tune memory\n"
            "  vllm_probe --scenario chatbot --tp 2 --gpu-ids 2,3 --gpu-mem 0.90\n\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'custom' = user-defined (requires --input-tokens, --output-tokens). "
            "'all' = run analyst, creative, chatbot sequentially."
        ),
    )
    ...
    physics.add_argument(
        ...
        help=(
            ...
            "'worst-case' = 250ms (maximum realistic). "
            "'custom' = user-defined (requires --delay-ms)."
        ),
    )
    ...
    custom = parser.add_argument_group(
        "Custom Scenario",
        "Parameters for --scenario custom (ignored for built-in scenarios)",
    )
    custom.add_argument(
        "--input-tokens",
        type=int, default=None,
        help="Input token count (required with --scenario custom)",
    )
    custom.add_argument(
        "--output-tokens",
        type=int, default=None,
        help="Output token count (required with --scenario custom)",
    )
    custom.add_argument(
        "--concurrent",
        type=int, default=None,
        help="Concurrent requests (default: 1, only with --scenario custom)",
    )

    # ── Custom Network Parameters ────────────────────────────────────────
    custom_net = parser.add_argument_group(
        "Custom Network",
        "Parameters for --network custom (ignored for built-in profiles)",
    )
    custom_net.add_argument(
        "--delay-ms",
        type=float, default=None,
        help="One-way delay in ms (required with --network custom)",
    )
    custom_net.add_argument(
        "--jitter-ms",
        type=float, default=None,
        help="Jitter in ms (default: 0, only with --network custom)",
    )

    # ── Input Mode ───────────────────────────────────────────────────────
    input_mode = parser.add_argument_group(
        "Input Mode",
        "Control how input prompts are generated",
    )
    input_mode.add_argument(
        "--random-input",
        action="store_true", default=False,
        help="Use random token IDs instead of repeated sentence. More realistic prefill.",
    )
    input_mode.add_argument(
        "--seed",
        type=int, default=42,
        help="Random seed for --random-input (default: 42)",
    )
    input_mode.add_argument(
        "--prompt-file",
        type=str, default=None,
        help="Path to .txt file to use as prompt. Most realistic input mode.",
    )
    ...
    parser.add_argument(
        ...
        help="GPU memory utilization 0.0-1.0 (default: 0.70). Raise for chatbot, lower if OOM.",
    )
...
# Validate custom scenario
    ...
    if args.network_profile == "custom":
        if args.delay_ms is None:
            parser.error("--network custom requires --delay-ms")
        LATENCY_PROFILES["custom"]["delay_ms"] = args.delay_ms
        LATENCY_PROFILES["custom"]["jitter_ms"] = args.jitter_ms if args.jitter_ms else 0.0
        LATENCY_PROFILES["custom"]["description"] = (
            f"Custom: {args.delay_ms}ms ± {args.jitter_ms or 0}ms"
        )

    scenarios_to_run = (
        [k for k in WORKLOAD_SCENARIOS.keys() if k != "custom"]
    ...
    if args.gpu_ids != "all":
        ...
        num_visible = len(args.gpu_ids.split(","))
        if num_visible < args.tensor_parallel:
            parser.error(
                f"--gpu-ids provides {num_visible} GPU(s) but "
                f"--tp {args.tensor_parallel} requires at least "
                f"{args.tensor_parallel} GPU(s)"
            )
